...
What it checks, executably, right now (the DOCUMENT checks):
  ...
  * Article IX's open matters are numbered contiguously, each carries a
    Status, and every "problem N" cross-reference resolves.
  * The tension protocol and the refuse-or-flag list are present and intact.
  * The normative text stays product-agnostic: product names may appear
    only inside Schedule A (the steward's descriptive bindings — the
    instance, never the norm). The document is PII-free (no email
    addresses beyond the organizational footer).
...
REGISTRY = [
    ...
    Invariant("REC-6", "Record", "a countersigner confers no authority",
              "implementation", "record-orchestrator tests: relay cannot gate or adjudicate"),
    ...
    Invariant("REC-8", "Record", "filing commitment is made at an independent countersigner",
              "implementation", "filing tests: operator absent at claim creation; single-write countersigning point"),
    ...
    Invariant("GOV-13", "Governance", "federates automatically",
              "implementation", "orchestrator tests: automatic federation and reporting to the federation board in the governance frontend"),
    # Legibility ladder
...
# Product names that must never appear in the normative text. They are
# permitted only inside the labeled "NTARI substack" subsections, which
# describe the instance, never the norm (see strip_substack_sections).
...
def check_glossary_binding(doc):
    gloss = section(doc, "## Article I", ["\n## Article II"])
    if gloss is None:
        return Result("D1 glossary bound to body", False, "no '## Article I' section found")
    body = doc[doc.find("## Article II"):]
...
def check_open_problems(doc):
    part = section(doc, "## Article IX", ["\n## Schedule A"])
    if part is None:
        return Result("D6 open problems intact", False, "no Article IX")
...
def strip_substack_sections(doc):
    """Remove Schedule A — the labeled exception where the instance's
    product names are permitted (descriptive bindings, never the norm).
    Skipping ends at the next heading of any level."""
    out, skipping = [], False
    for line in doc.splitlines(True):
        if re.match(r"^#{2,4} ", line):
            skipping = bool(re.match(r"^#{2,4} Schedule A", line))
        if not skipping:
            out.append(line)
    return "".join(out)
...
def check_product_agnosticism(doc):
    scan = strip_substack_sections(doc)
    hits = [name for name in PRODUCT_NAMES
            if re.search(r"(?<![A-Za-z])" + re.escape(name) + r"(?![A-Za-z])", scan)]
    return Result("D9 product names confined to substack subsections", not hits,
                  ("product names in normative text: " + ", ".join(hits)) if hits else "")
